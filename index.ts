...
app.get("/quote", (req, res) => {
    ...
    if (side === "bid") {
      ...
      for (let order of sortedAsks) {
        ...
        let fillQty = Math.min(order.quantity, remaining);
        total += fillQty * order.price;
        remaining -= fillQty;
      }
      if (remaining > 0) {
        return res.status(400).json({ error: "Not enough ask orders" });
      }
      return res.json({ quote: total });
    }
    ...
    if (side === "ask") {
      ...
      if (remaining > 0) {
        return res.status(400).json({ error: "Not enough bid orders" });
      }
      return res.json({ quote: total });
    }
